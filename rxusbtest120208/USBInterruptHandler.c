...
void HandleBRDY(void)
{
	...
	//
	if((USB0.BRDYSTS.BIT.PIPE2BRDY == 1) && (USB0.BRDYENB.BIT.PIPE2BRDYE == 1))
	{
		DEBUGFIFO_OutLine("Pipe2 Interrupt.");
	...
	//
	if((USB0.BRDYSTS.BIT.PIPE6BRDY == 1) && (USB0.BRDYENB.BIT.PIPE6BRDYE == 1))
	{
		DEBUGFIFO_OutLine("Pipe6 Interrupt.");

		/*Clear this bit (write 1 to all other bits)*/
		USB0.BRDYSTS.WORD = ~0x0040;
