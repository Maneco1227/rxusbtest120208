...
void SetConfiguration(void)
{
	...
	USB0.D1FIFOSEL.BIT.MBW = 1;
	USB0.D1FIFOSEL.BIT.CURPIPE = 1;
	...
	/***Interrupt IN***/	
	/*PID must be set to NAK before configuring PIPECFG*/
	USB0.PIPE6CTR.BIT.PID = 0;
	
	/*Wait for pipe to be not busy*/
	while(USB0.PIPE6CTR.BIT.PBUSY == 1);

	/*Reset Data toggle*/
	USB0.PIPE6CTR.BIT.SQCLR = 1;	

	/*Clear Buffer*/
	USB0.PIPE6CTR.BIT.ACLRM = 1;
	USB0.PIPE6CTR.BIT.ACLRM = 0;
}
